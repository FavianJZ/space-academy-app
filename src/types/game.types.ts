...
export interface GameState {
  ...
  // --- Backend integration ---
  // playerId/authToken persist alongside everything else in this store
  // (same localStorage-backed `persist` middleware), so a returning
  // player stays logged in across refreshes without a separate login
  // step.
  ...
  /**
   * Creates-or-finds the player on the backend using the current
   * playerData/character, stores the returned id + bearer token, and
   * hydrates local game state from whatever the server already knows
   * about this player (important for a returning player opening the
   * game on a new device/browser - their server-side progress becomes
   * the local state, not the other way around). Throws on failure so
   * the caller can show an error instead of silently proceeding
   * offline.
   */
  registerOrLoginPlayer: () => Promise<void>;
  /** Clears auth only (playerId/token) - local game progress stays. */
  logout: () => void;
  ...
  /** Submits accumulated boss damage from one stage-6 session. Fire
   * -and-forget from the caller's point of view - errors are logged,
   * not thrown, so a flaky connection never blocks the completion
