...
const Bedroom: React.FC = () => {
  ...
  const registerOrLoginPlayer = useGameStore(
    (state) => state.registerOrLoginPlayer
  );
  const isRegistering = useGameStore((state) => state.isRegistering);
  ...
  const handleFinalAuthentication = () => {
    ...
    // registerOrLoginPlayer reads playerData/character from the store,
    // so it runs right after setPlayerData above (zustand applies that
    // set() synchronously, so the values are already current here).
    registerOrLoginPlayer()
      ...
      .catch(() => {
        // registerError is already set inside the store action - show
        // it and let the player retry from the name step instead of
        // silently stranding them on a "submitted" screen that never
        // navigates anywhere.
        setStepError(
          "Gagal menghubungkan ke server. Cek koneksi internet kamu, lalu coba lagi."
        );
