...
const WarpTunnel = ({ count = 320, warpSpeed, isAlarmActive, phase }: WarpTunnelProps) => {
  ...
  useFrame(({ clock }, delta) => {
    ...
    const positionBuffer = geometryRef.current.getAttribute('position');
    if (!(positionBuffer instanceof THREE.BufferAttribute)) {
      return;
    }
    const positionAttribute = positionBuffer.array as Float32Array;
