...
const Stage5LogicFlow: React.FC<Stage5LogicFlowProps> = ({ planetId }) => {
    ...
    const handleTiltMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        setTilt({ x: y * 5, y: -x * 5 }); // Less rotation for circuit board since it's big
    };
    ...
    return (
        <div className={`stage-logic-flow blueprint-theme ${screenEffect}`} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onTouchMove={handleTouchMove as any} onTouchEnd={handleTouchEnd as any}>
            ...
            <div 
              className="circuit-board hud-3d-card" 
              ref={containerRef}
                ...
                <div className={`feedback-modal ${status}`}>
                    ...
                    <div className="hud-sweep-btn-wrapper" style={{ marginTop: '10px' }}>
                      {status === 'success' ? (
                          ...
                          <button className="retry-btn hud-sweep-btn" onClick={handleRetryLevel}>TRY AGAIN</button>
                      )}
                    </div>
                </div>
            )}
