...
const Stage3PuzzleGame: React.FC<Stage3PuzzleGameProps> = ({ planetId }) => {
    ...
    useEffect(() => {
        const frameId = window.requestAnimationFrame(initializePuzzle);
    ...
    }, [isPuzzleSolved, playSfx, restartKey, showCompletion, tiles.length]);
    ...
    }, [handleComplete, isPuzzleSolved, showCompletion, tiles.length, timeLeft]);
                ...
                >
                    ...
                    <div className="puzzle-stats-bar">
                        ...
                        <button className={`reference-btn ${showReference ? 'active' : ''}`} onClick={() => setShowReference(!showReference)}>
                            {showReference ? 'Hide Reference' : 'View Reference'}
                        </button>
                    </div>
