...
const Stage6BugHunt: React.FC<Stage6BugHuntProps> = ({ planetId }) => {
  ...
  const playSound = useCallback(
    (
      ...
      try {
        ...
        audio.volume =
          sfxVolume * (type === "miss" ? 0.3 : type === "laser" ? 0.5 : 1);
        audio.play().catch(() => {});
      } catch {}
    },
    [sfxVolume]
  ...
  const spawnSnippet = useCallback(() => {
    ...
    if (activeCount >= diff.maxActive) {
      spawnTimerRef.current = window.setTimeout(spawnSnippet, 500);
      return;
    }
  ...
  const triggerAbduction = useCallback(() => {
    ...
    abductionTimerRef.current = window.setTimeout(triggerAbduction, nextDelay);
  }, [isBossMode]);
    ...
    (cellIdx: number, playerId: CoopPlayerId = "P1") => {
      ...
      if (snippet.isBug) {
        ...
        if (isBossMode) {
          ...
          setUfoHitFlash(true);
          window.setTimeout(() => setUfoHitFlash(false), 200);

          setLaserActive(true);
          window.setTimeout(() => setLaserActive(false), 300);
                ...
                <div className="tutorial-cards-grid">
                  <div className="tutorial-card">
                    <div className="tutorial-card-header">
                      <span className="tutorial-card-badge">🖱️</span>
                      <div>
                  ...
                  <div className="tutorial-card">
                    <div className="tutorial-card-header">
                      <span className="tutorial-card-badge">🔢</span>
                      <div>
            ...
            >
              ...
              {isBossMode && (
                <div className="boss-raid-note">
                  <span className="boss-raid-note-icon">👥</span> All players
